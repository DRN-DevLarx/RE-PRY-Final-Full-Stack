import React from 'react';
import '../styles/PublicDesact.css';
import { useNavigate } from 'react-router-dom';
import { CerrarDashboard } from './CerrarDashboard';

function PublicDesact() {
  const navigate = useNavigate();

  function volver() {
    CerrarDashboard(navigate)
  }

  return (
    <div id='ContUltimasPublicaciones'>
      <div className='headerUltimasPublicaciones'>
        <h3>Publicaciones desactivadas</h3>
        <svg onClick={volver} xmlns="http://www.w3.org/2000/svg" width="30" height="30" fill="white" className="bi bi-box-arrow-right" viewBox="0 0 16 16">
          <path d="M10 12.5a.5.5 0 0 1-.5.5h-8a.5.5 0 0 1-.5-.5v-9a.5.5 0 0 1 .5-.5h8a.5.5 0 0 1 .5.5v2a.5.5 0 0 0 1 0v-2A1.5 1.5 0 0 0 9.5 2h-8A1.5 1.5 0 0 0 0 3.5v9A1.5 1.5 0 0 0 1.5 14h8a1.5 1.5 0 0 0 1.5-1.5v-2a.5.5 0 0 0-1 0z"/>
          <path d="M15.854 8.354a.5.5 0 0 0 0-.708l-3-3a.5.5 0 0 0-.708.708L14.293 7.5H5.5a.5.5 0 0 0 0 1h8.793l-2.147 2.146a.5.5 0 0 0 .708.708z"/>
        </svg>
      </div>

      <main className='Cont'>
        <div id='containerOf'>
          <article>Cuadro 1</article>
          <article>Cuadro 2</article>
          <article>Cuadro 3</article>
          <article>Cuadro 4</article>
          <article>Cuadro 5</article>
          <article>Cuadro 6</article>
        </div>
      </main>
    </div>
  );
}

export default PublicDesact;
